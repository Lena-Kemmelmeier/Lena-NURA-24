...
% GUI
% Put 0 for order settings! Get rid of this GUI entry later
...
prompt = {'Enter participant number:','Enter mouse settings:'};
...
test_path = char(root + "/Raw" + '/' + strcat(ID,'_RecallRecogPractice_raw.mat'));

% if exist(test_path,'file') == 2 % 2 if file exists, 0 if it doesn't
%     error(duplicateSubID);
% end

output_name = sprintf('%s/Raw/%s_RecallRecogPractice_raw',root,ID);
data_name = sprintf('%s/Data/%s_RecallRecog_dataPractice',root,ID);
ana_name = sprintf('%s/Analyzed/%s_RecallRecogPractice_ana',root,ID);
            ...
            for t = 1:nTrialsTotal % start trial loop
              ...
              if trialType == 1 % this is a recall trial
                  ...
                  % each color must be at least 30 degrees apart from one another (don't want colors to look exactly the same!)
                    ...
                    diffColorInDegrees = first_num - 45; % change this for mismatch difference!
